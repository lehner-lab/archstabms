...
#'
#' Linear model from single mutant effects (in WT background) from shallow ddPCA data.
#'
...
#' @export
#' @import data.table
archstabms_ddPCA_singlebackground_linear_model <- function(
  mochi_outpath,
  ddpca_outpath,
  position_offset,
  load_mochi_data,
  ...
  #Display status
  message(paste("\n\n*******", paste("running stage: archstabms_ddPCA_singlebackground_linear_model for", domain_name), "*******\n\n"))
  ...
  #Load singles
  load(ddpca_outpath)
  singles <- singles[STOP==F]
  singles[, id := paste0(WT_AA, Pos-position_offset, Mut)]
  ...
  for(i in names(mm)[!grepl("fitness", names(mm))]){
    if(i=='WT'){
      ...
      mm_ddpca_list[[i]][, fitness := 0]
    }else if(singles[id==i,.N]!=0){
      mm_ddpca_list[[i]] <- copy(mm_ddpca)
      mm_ddpca_list[[i]][, fitness := singles[id==i,fitness]]
  ...
  #Model results
  coef_dt <- data.table()
  mm_predict <- predict(ddpca_lm, newdata = mm)
  for(i in 1:10){
    #Predict
