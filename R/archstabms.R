...
archstabms <- function(
	...
	archstabms_thermo_model_results_datasets(
		...
		ddPCA_free_energies_list = list(
			"CM1" = file.path(base_dir, "Data", "mochi", "ddPCA", "mochi__fit_tmodel_3state_sparse_dimsum128", "model_weights_0.txt"),
			"CM1binding" = file.path(base_dir, "Data", "mochi", "ddPCA", "mochi__fit_tmodel_3state_sparse_dimsum128", "model_weights_0.txt"),
			"CM2" = file.path(base_dir, "Data", "mochi", "ddPCA", "mochi__fit_tmodel_3state_sparse_dimsum128", "model_weights_0.txt"),
			"CM6" = file.path(base_dir, "Data", "mochi", "ddPCA", "mochi__fit_tmodel_3state_sparse_dimsum128", "model_weights_0.txt"),
			"SRCN_CM1" = NA),
		base_dir = base_dir,
		output_dir = output_dir,
		stagenum = stagenum,
	...
	archstabms_structure_metrics_datasets(
		dataset_names = c(
			...
			"SRCN_CM1"),
		pdb_file_list = list(
			"CM1" = file.path(base_dir, "Data", "pdb", "2vwf.pdb"),
			"CM1binding" = file.path(base_dir, "Data", "pdb", "2vwf.pdb"),
			"CM2" = file.path(base_dir, "Data", "pdb", "2vwf.pdb"),
			"CM6" = file.path(base_dir, "Data", "pdb", "2vwf.pdb"),
			"SRCN_CM1" = file.path(base_dir, "Data", "pdb", "2src.pdb")),
		base_dir = base_dir,
		output_dir = output_dir,
		stagenum = stagenum,
		execute = (first_stage <= stagenum & last_stage >= stagenum))
