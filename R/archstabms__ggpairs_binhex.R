...
#' @return Nothing
#' @export
archstabms__ggpairs_binhex <- function(
  input_dt, 
  output_file, 
  input_dt_upper=NULL,
  width = 12, 
  ...
  if(dim(input_dt)[1]==0){
    warning("archstabms__ggpairs_binhex.R: No data to plot (empty data.table 'input_dt').", call. = FALSE, immediate. = TRUE, noBreaks. = TRUE)
    return(NULL)
  }
